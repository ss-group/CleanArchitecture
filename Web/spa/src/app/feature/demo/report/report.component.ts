...
import { Page, ModalService, FormUtilService, RowState } from "@app/shared";
import { ReportService, ReportParam } from "./report.service";
...
radioItems = [{ value: 'pdf', text: 'PDF' }, { value: 'xlsx', text: 'Excel' }]
  ...
  print() {
    ...
    this.rs.generateReport(this.reportParam).pipe(
      finalize(() => {
        this.printing = false;
      })
    )
