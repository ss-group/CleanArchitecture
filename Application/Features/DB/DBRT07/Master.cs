...
namespace Application.Features.DB.DBRT07
{
    public class Master
    {
        ...
        {
            public IEnumerable<dynamic> PreTypes { get; set; }
        }
        ...
        public class Handler : IRequestHandler<Query, MasterData>
        {
            ...
            public async Task<MasterData> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var preTypes = await this.GetPreName(cancellationToken);

                return new MasterData { PreTypes = preTypes };
            }

            private Task<IEnumerable<dynamic>> GetPreName(CancellationToken cancellationToken)
            {
                StringBuilder sql = new StringBuilder();
                sql.AppendLine("SELECT get_name(@lang, list_item_name_tha, list_item_name_eng) AS text, list_item_code AS value");
                sql.AppendLine(", active AS active ");
                sql.AppendLine("FROM db_list_item ");
                sql.AppendLine("WHERE list_item_group_code = 'PreNameType' ");
